...
- **SEC-RANDOM-2** — All random calls must route through one seam. Two draws
  stay outside it: the context blinding of SEC-RANDOM-3, and the file names that
  `mkstemp(3)` draws for [STORE-ATOMIC-3](storage.md#store-atomic). Only regress
